...
function isClothingDetails(product: ProductDetails): product is ClothingDetails {
  return (
    product.type === 'clothing'
  )
}

function isApplianceDetails(product: ProductDetails): product is ApplianceDetails {
  return (
...
export function loadProductsFetch(): Promise<void> {
  ...
  const promise = fetch(
    ...
    products = productsData.map((productDetails: ProductDetails) => {
      if (isClothingDetails(productDetails)) {
        return new Clothing(productDetails);
      } else if (isApplianceDetails(productDetails)) {
...
export function loadProducts(fun: () => void): void {
  const xhr = new XMLHttpRequest();

  xhr.addEventListener('load', () => {
    products = JSON.parse(xhr.response).map((productDetails: ProductDetails) => {
      if (isClothingDetails(productDetails)) {
        return new Clothing(productDetails);
      } else if (isApplianceDetails(productDetails)) {
        return new Appliance(productDetails);
...
export const products: Product[] = [
  ...
  if (productDetails.type === 'clothing') {
    return new Clothing({
      ...productDetails,
      sizeChartLink: productDetails.sizeChartLink || `images/clothing-size-chart.png`
    });
